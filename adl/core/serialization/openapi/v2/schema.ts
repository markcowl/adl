...
export async function newProcessSchema(schema: v2.Schema|v2.SchemaReference, $: Context<v2.Model>, options?: { isAnonymous?: boolean }): Promise<TypeReference> {
  ...
  const impl = async () => {
    if( isReference(schema) ) {
      ...
      if( !options?.isAnonymous) {
        ...
        typeRef = newCreateTypeAlias($.api, nameOf(schema), typeRef, commonProperties(<v2.Schema>schema));
      }
